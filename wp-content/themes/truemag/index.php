	...
	<div class="container">
		<div class="row">
			...
			<div id="content" class="<?php echo $layout != 'full' ? ($sidebar_width ? 'col-md-9' : 'col-md-8') : 'col-md-12' ?><?php echo ($layout == 'left') ? " revert-layout" : ""; ?>" role="main">
				...
				<?php if (have_posts()) : ?>
					<section class="<?php echo $style == 'blog' ? 'blog-listing' : 'video-listing';
									echo ot_get_option('default_listing_layout') ? ' ' . ot_get_option('default_listing_layout') : ''; ?>">
						<?php if ($style == 'video') { ?>
							<div class="video-listing-head">
								<?php
								if (is_home()) {
									if (ot_get_option('show_blog_title', '1')) { ?>
										<h2 class="light-title"><?php global $global_title;
																echo $global_title ?></h2>
									<?php }
								} elseif (!is_author()) { ?>
									<h2 class="light-title"><?php global $global_title;
															echo $global_title ?></h2>
						...
						<?php } ?>
						<div class="<?php echo $style == 'blog' ? 'blog-listing-content' : 'video-listing-content' ?> <?php if ($pagination == 'page_ajax' || $pagination == '') {
																															echo 'tm_load_ajax';
																														} ?>  ">
							<?php
							if ($style == 'blog') {
						...
						<!--/video-listing-content(blog-listing-content)-->
						<div class="clearfix"></div>
