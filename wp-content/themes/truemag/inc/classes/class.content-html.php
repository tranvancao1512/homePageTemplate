	...
	class CT_ContentHtml{
			  ...
			  <div class="video-item">
				  <div class="videos-row">
						  <div class="item-thumbnail">';
                            $html .= do_shortcode('[cactus_badges]');
							$html .= '<a href="'.get_permalink().'" title="'.esc_attr(get_the_title()).'">';
		...
		function get_item_video($thumb, $show_title, $show_rate, $show_dur, $themes_pur, $quick_view='def'){	
		  ...
		  $html .= '
				...
				">';}
			$html .= '	
				<div class="video-item format-' . $format . '">
					<div class="item-thumbnail">';
                    $html .= do_shortcode('[cactus_badges]');
                    $html .= '<a href="'.get_permalink().'" title="'.esc_attr($title).'">';
							if(has_post_thumbnail()){
                                $responsive_mode = ot_get_option('responsive_image', 'on');
                                if($responsive_mode == 'on'){
		...
		function get_item_smart_video($thumb){	
				$html='';
				$html .= '
					  <div class="video-item format-' . get_post_format() . '">
						  <div class="item-thumbnail">';
                            $html .= do_shortcode('[cactus_badges]');
							$html .=  '<a href="'.get_permalink().'" title="'.esc_attr(get_the_title()).'">';
							if(has_post_thumbnail()){
		...
		function tm_likes_html($post,$like_count,$themes_pur,$show_likes,$show_com,$show_rate,$show_view,$show_excerpt,$excerpt){
				  $widget_data .='
				  <div class="col-md-12 col-sm-4">
					<div class="video-item format-' . get_post_format($post->post_id) . '">
						<div class="videos-row">
							<div class="item-thumbnail">';
                            $widget_data .= do_shortcode('[cactus_badges]');
								  $widget_data .= '<a href="'.get_permalink($post->post_id).'" title="'.the_title_attribute('echo=0').'">';
		...
		function tm_likes_cat_html($post,$like_count,$format_cat){
				$widget_data .= '   
					<div class="video-item format-' . get_post_format($post->post_id) . '">
						<div class="item-thumbnail">';
                            $html .= do_shortcode('[cactus_badges]');
							$html .= '<a href="'.get_permalink($post->post_id).'" title="'.esc_attr(get_the_title()).'">';
								  if(has_post_thumbnail($post->post_id)){
                                            $responsive_mode = ot_get_option('responsive_image', 'on');
                                            if($responsive_mode == 'on'){
                                                $widget_data .= cactus_thumbnail('thumb_139x89', $post->post_id);
		...
		function get_item_related($postformat, $themes_pur){	
		  $html='';
			$html .= '
			<div class="col-md-12 col-sm-4">
			  <div class="video-item format-' . $postformat . '">
				 <div class="videos-row">
						  <div class="item-thumbnail">';
                          $html .= do_shortcode('[cactus_badges]');
							$html .= '<a href="'.get_permalink().'" title="'.esc_attr(get_the_title()).'">';
