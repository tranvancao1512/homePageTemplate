...
class GF_Field_VS_Tags extends GF_Field {
	...
	public function get_form_editor_field_title() {
		return esc_html__( 'Post Tags', 'cactusthemes' );
	}
	...
	function validate( $value, $form ) {
		...
		if ( $this->isRequired ) {
			...
			if ( empty($value) ) {
				...
				$this->validation_message = empty( $this->errorMessage ) ? esc_html__( 'This field is required. Please enter at least one tag', 'cactusthemes' ) : $this->errorMessage;
			}
		}
        
        $limit_tags = intval(ot_get_option('user_submit_limit_tag', 'cactusthemes'));
            
        if($limit_tags != 0){
            $tags = explode(',',$value);
            if(count($tags) > $limit_tags){
                $this->failed_validation  = true;
                $this->validation_message = empty( $this->errorMessage ) ? sprintf(esc_html__( 'You are limited to enter %d tags', 'cactusthemes' ), $limit_tags) : $this->errorMessage;
            } 
        }
	}
