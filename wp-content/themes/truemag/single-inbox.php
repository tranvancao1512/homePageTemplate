...
$delay_video = $delay_video ? cactus_utube_conver_time( $delay_video ) : 0;
$delay_video *= 1000;
...
if($force_videojs == 'on' && $single_player_video == 'videojs' && (!$is_iframe && in_array($video_channel_url, array('youtube', 'vimeo', 'dailymotion')))){
	// do nothing
} else {
...
<div class="single-inbox">
            	...
            	<div class="video-player <?php echo ot_get_option('single_layout_video', 'full_width') == 'inbox' ? 'player-layout-inbox' : 'player-layout-full_width';?> <?php echo $has_sidebar ? 'has-sidebar' : 'no-sidebar';?>">
                	<div class="player-content <?php if($c_jw7_ex == '1'){ echo ' jw7-plr ';}?>">
                    	...
                    	<div id="player-embed" <?php if(!$is_iframe && strpos($url, 'facebook.com') !== false){?> class="fb-video-ct" <?php }?>>
							...
							if($single_player_video == 'jwplayer' && $c_jw7_ex == '1' && ($video_channel_url == 'youtube' || $video_source == 'self-hosted')){
								cactus_jwplayer7();
                                ...
                                <?php
							}elseif($force_videojs == 'on' && $single_player_video == 'videojs' && (!$is_iframe && ( in_array($video_channel_url, array('youtube','vimeo','dailymotion'))))){
								...
								if((!$is_iframe && strpos($url, 'wistia.com') !== false )|| (strpos($code, 'wistia.com') !== false ) ){
									...
									<?php 
									 ...
									 if(( $video_channel_file == 'youtube' ) && ($using_yt_param == 1) || (!$is_iframe && $video_channel_url == 'youtube' ) && ($using_yt_param == 1) && (!$detect->isTablet())){?>
									 <div class="obj-yt-inbox">
										<object width="900" height="506">
										<param name="movie" value="//www.youtube.com/v/<?php echo Video_Fetcher::extractIDFromURL($url); ?><?php echo $youtube_start_time != '' ? '&start=' . $youtube_start_time : '';?><?php if($onoff_related_yt!= '0'){?>&rel=0<?php }if($auto_play=='1'){?>&autoplay=1<?php }if($onoff_info_yt=='1'){?>&showinfo=0<?php }if($remove_annotations!= '1'){?>&iv_load_policy=3<?php }if($onoff_html5_yt== '1'){?>&html5=1<?php }?>&wmode=transparent&autohide=1<?php if($youtube_start!= ''){?>&start=<?php echo esc_attr($youtube_start);}?><?php if($youtube_end!= ''){?>&end=<?php echo esc_attr($youtube_end);}?>" ></param>
									 ...
									 <?php	 
										 ...
										 if(($single_player_video == 'jwplayer' && class_exists('JWP6_Plugin') && $video_captions && ($file != '')) || ($single_player_video == '' && class_exists('JWP6_Plugin') && $video_captions && ($file != ''))){
											 // do nothing
		...
		<?php } /* end of default Video Player */ ?>
        ...
        if (have_posts()) :
            ...
            <div class="box-title hentry">
            	<div class="title-info">
                    ...
                    <?php if(is_single()){ ?>
                    <div class="item-info">
						<?php if(ot_get_option('single_show_meta_author',1)){?>
...
//        }
		...
		<div class="hidden">
			...
			<span itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating">
				 <?php echo esc_html__('Rating:','cactusthemes');?> <span itemprop="ratingValue"><?php echo round($review_point/10,1) ?></span> / <meta itemprop="bestRating" content="10"/>10
			</span>
		</div>
		<?php } else { echo '>'; } ?>
